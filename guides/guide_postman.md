...
> 
- В директории target находится исполняемый jar-файл ```airline-project.jar``` (предварительно в maven делаем ```clean/install```). 
- Нам в терминале нужно перейти в эту директорию <code>cd airline-project/target/</code>. 
- Далее запускаем jar-файл приложения с профилем *noSecurity* <code>java -jar airline-project.jar --spring.profiles.active=noSecurity</code>
