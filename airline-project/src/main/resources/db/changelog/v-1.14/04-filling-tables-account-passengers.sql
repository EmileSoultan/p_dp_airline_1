INSERT INTO account (first_name, last_name, birth_date, phone_number, email, password, security_question, answer_question, id)
...
VALUES ((SELECT account.id FROM account WHERE account.email = 'admin@mail.ru'),
        (SELECT roles.id FROM roles WHERE roles.name = 'ROLE_ADMIN'));

INSERT INTO user_roles(users_id, roles_id)
VALUES ((SELECT account.id FROM account WHERE account.email = 'manager@mail.ru'),
